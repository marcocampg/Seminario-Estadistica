...
def plot_elbow_curve(clf, X, title='Elbow Plot', cluster_ranges=None, n_jobs=-1,
                     show_cluster_time=True, ax=None, figsize=(12,5),
    ...
    ax.plot(cluster_ranges, clfs, '*-', lw = 3, color = 'steelblue')
    ...
    if show_cluster_time:
        ax2_color = 'darkred'
        ax2 = ax.twinx()
        ax2.plot(cluster_ranges, times, ':', alpha=0.75, color=ax2_color, lw = 3)
        ax2.set_ylabel('Clustering duration (seconds)',
                       color=ax2_color, alpha=0.75,
                       fontsize=text_fontsize, fontfamily = 'serif', labelpad = 15)
        
        ax2.tick_params(colors = ax2_color, labelsize=text_fontsize -2, axis = 'both')
    ...
    #ax.set_xticklabels(ax.get_xticks(), color = '#525252',
     #                 fontsize=text_fontsize -2)
    
    ax.tick_params(colors = '#525252', labelsize=text_fontsize -2, axis = 'both')
...
def _clone_and_score_clusterer(clf, X, n_clusters):
    ...
    return clf.fit(X).inertia_, time.time() - start
