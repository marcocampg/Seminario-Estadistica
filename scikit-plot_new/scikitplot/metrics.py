...
def plot_confusion_matrix(y_true, y_pred, labels=None, names = None, true_labels=None,
                          pred_labels=None, title=None, normalize=False,
                          hide_zeros=False, hide_counts=False, x_tick_rotation=0, ax=None,
                          figsize=(8,6), cmap='Blues', title_fontsize=18,
                          text_fontsize=14):
    ...
    Args:
        ...
        names (list of names of classes):
            List of names of classes where show in the matrix confusion.
            All classes you should put.
        ...
        true_labels (array-like, optional): The true labels to display
            (str array if names is not None).
    ...
    if names is None:
        ...
        try:
            labels = [names.index(x) for x in sorted(labels)]
        except:
            pass
    ...
    if true_labels is None:
        ...
        try:
            true_labels = [names.index(x) for x in true_labels]
        except:
            pass
    ...
    if pred_labels is None:
        ...
        try:
            pred_labels = [names.index(x) for x in pred_labels]
        except:
            pass
    ...
    if title:
        ax.set_title(title, fontsize=title_fontsize, pad = 20, 
                     fontfamily = 'serif', color = '#4f4f4f')
        
    elif normalize:
        ax.set_title('Normalized Confusion Matrix', fontsize=title_fontsize,
                    pad = 20, fontfamily = 'serif', color = '#4f4f4f')
    else:
    ...
    bar = plt.colorbar(mappable=image, ticks = [cm.min(), cm.max()])
    x_tick_marks = np.arange(len(pred_classes))
    y_tick_marks = np.arange(len(true_classes))
    
    ax.set_xticks(x_tick_marks, )
    if names is not None:
        pred_classes = [names[index] for index in pred_classes]
    ax.set_xticklabels(labels = pred_classes, fontsize=text_fontsize,
                       rotation=x_tick_rotation, fontfamily = 'serif', color = '#525252')
    
    ax.set_yticks(y_tick_marks)
    if names is not None:
        true_classes = [names[index] for index in true_classes]
    ax.set_yticklabels(labels = true_classes, fontsize = text_fontsize,
        ...
        for i, j in itertools.product(range(cm.shape[0]), range(cm.shape[1])):
            if not (hide_zeros and cm[i, j] == 0):
                ax.text(j, i, cm[i, j],
                        ...
                        fontfamily = 'serif',
                        color="white" if cm[i, j] > thresh else '#525252')
   
    bar.ax.set_yticklabels(['Low Value','High Value'],
                           fontsize = text_fontsize, fontfamily = 'serif', color = '#525252')
...
def plot_roc(y_true, y_probas, title='ROC Curves',
             plot_micro=False, plot_macro=False, label_model = None, 
             opt_thres = False, plot_ot = False, multiclass = False, 
             classes_to_plot=None, ax=None, figsize=(7,7), color = None,
             title_fontsize=18, text_fontsize=14, bbox_to_anchor = (1.67,.89),
             **kwargs):
    ...
    Args:
        ...
        label_model (str, optional): Name of model 
        
        opt_thres (Bool, optiona): Print optimal thresholds, tpr and fpr
        
        plot_ot (Boolean, optional): Plot a circle in the optimal threshold
        
        multiclass (boolean, optional): If only you have multiclass (class>2)
    ...
    if multiclass:
        ...
        indices_to_plot = np.array([True])
        probas = probas.reshape((probas.shape[0],-1))
        classes = np.ones(classes.size, dtype = int)
    ...
    for i, to_plot in enumerate(indices_to_plot):
        fpr_dict[i], tpr_dict[i], thresholds[i] = roc_curve(y_true, probas[:, i],
                                                pos_label=classes[i])
        if to_plot:
            ...
            if label_model is None:
                label_model_  = 'Class {} \n(AUC = {:0.5f}) (Gini = {:0.5f})'\
                                 .format(classes[i], roc_auc, 2* roc_auc - 1)
            else:
                label_model_ = label_model + '\n(AUC = {:0.5f}) (Gini = {:0.5f})'\
                                 .format(roc_auc, 2* roc_auc - 1)
            
            ax.plot(fpr_dict[i], tpr_dict[i], lw = 3, color = color,
                    label = label_model_)
            
            closest_zero = np.argmin(fpr_dict[i] + (1 - tpr_dict[i]))
            opt_threshold = thresholds[i][closest_zero]
            closest_zero_fpr = fpr_dict[i][closest_zero]
            closest_zero_tpr = tpr_dict[i][closest_zero]
            
            if opt_thres:
                print('*'*50)
                if label_model is None:
                    print(' '*10 + 'Class {}'.format(classes[i]))
                else:
                    print(' '*10 + label_model)
                print(' '*10 + 'Optimal threshold   ' + f'{opt_threshold:.8f}') 
                print(' '*10 + 'True Positive Rate  ' + f'{closest_zero_tpr:.8f}' )
                print(' '*10 + 'False Positive Rate ' + f'{closest_zero_fpr:.8f}' )
                print('*'*50)
                
            if plot_ot:
                plt.plot(closest_zero_fpr, closest_zero_tpr, 'o', markersize = 12, 
                         fillstyle = 'none', c='black', mew=3)
    ...
    ax.plot([0, 1], [0, 1], 'k:', lw=2, alpha  = .3)
    ...
    ax.set_xticklabels(x, color = '#525252',fontfamily = 'serif')
...
def plot_ks_statistic(y_true, y_probas, title='KS Statistic Plot',
                      ax=None, figsize = (7,7), title_fontsize=18,
                      text_fontsize=14, bbox_to_anchor = (1.65,.8),
                      **kwargs):
    ...
    ax.plot(thresholds, pct1, lw=3, label='Class {}'.format(classes[0]), color = 'red')
    ax.plot(thresholds, pct2, lw=3, label='Class {}'.format(classes[1]))
    
    idx = np.where(thresholds == max_distance_at)[0][0]
    sns.despine(trim = True, offset = 5)
    
    ax.axvline(max_distance_at, *sorted([pct1[idx] - .05, pct2[idx] +.05]),
               label='KS Statistic: {:.5f} at {:.5f}'.format(ks_statistic,
                                                             max_distance_at),
    ...
    ax.set_xticks(x, color = '#525252',fontfamily = 'serif')
    ax.set_yticks(x, color = '#525252',fontfamily = 'serif')
...
def plot_precision_recall(y_true, y_probas,
                          ...
                          plot_micro=False, label_model = None,
                          opt_thres = False, plot_ot = False,
                          multiclass = False,
                          classes_to_plot=None, ax=None,
                          figsize=(7,7), color = None,
                          title_fontsize=18,
                          text_fontsize=14, bbox_to_anchor = (1.0,.93),
                          **kwargs):
    ...
    Args:
        ...
        opt_thres (Bool, optional): Print optimal thresholds, tpr and fpr
    ...
    if multiclass:
        ...
        probas = probas.reshape((probas.shape[0],-1))
        classes = np.ones(classes.size, dtype = int)
        binarized_y_true = y_true.reshape((y_true.shape[0],-1))
    ...
    for i, to_plot in enumerate(indices_to_plot):
        if to_plot:
            ...
            if label_model is None:
                label_model_ = 'Class {} \n''(AP = {:0.5f})'.format(classes[i],
                                                                        average_precision)
            else:
                label_model_ = label_model + '\n(AP = {:0.5f})'.format(average_precision)
                
            ax.plot(recall, precision, lw=3,
                    label= label_model_, color=color)
            
            closest_zero = np.argmin(2 - precision - recall)
            opt_threshold = thresholds[closest_zero]
            ...
            if opt_thres:
                ...
                if label_model is None:
                    ...
                    print(' '*10 + label_model)
                print(' '*10 + 'Optimal threshold ' + f'{opt_threshold:.8f}') 
                print(' '*10 + 'Precision         ' + f'{closest_zero_p:.8f}' )
                print(' '*10 + 'Recall            ' + f'{closest_zero_r:.8f}' )
                print('*'*50)
            
            if plot_ot:
                plt.plot(closest_zero_r, closest_zero_p, 'o', markersize = 12, 
                         fillstyle = 'none', c='black', mew=3)
...
def plot_silhouette(X, cluster_labels, title='Silhouette Analysis',
                    metric='euclidean', copy=True, ax=None, figsize= (8,4),
                    cmap='tab10_r', title_fontsize=18,
                    text_fontsize = 14, bbox_to_anchor = (1.5,.7),
                      **kwargs):
    ...
    for i in range(n_clusters):
        ...
        ax.text(-0.05, y_lower + 0.5 * size_cluster_i, str(le.classes_[i]),
                fontsize=text_fontsize -2, color = '#525252',fontfamily = 'serif')
        
        y_lower = y_upper + 10

    ax.axvline(x=silhouette_avg, color="red", linestyle="--",
               label='Average Silhouette score: \n{0:0.5f}'.format(silhouette_avg))

    ax.set_yticks([])  # Clear the y-axis labels / ticks
    ax.set_xticks(np.arange(-0.1, 1, 0.2),  color = '#525252',fontfamily = 'serif')
    ...
    sns.despine(trim = True, offset = 5, left = True)
    
    leg = ax.legend(loc='best', fontsize=text_fontsize - 2, 
                    edgecolor = 'white',  bbox_to_anchor = bbox_to_anchor, 
                    **kwargs)
    leg.texts[0].set_color('#4f4f4f')
    leg.texts[0].set_family('serif')
...
def plot_calibration_curve(y_true, probas_list, clf_names=None, n_bins=10,
                           ...
                           ax=None, figsize=(7,7), cmap='tab10',
                           title_fontsize=18, text_fontsize=14,
                           bbox_to_anchor = (1,.78), **kwargs):
    ...
    for i, probas in enumerate(probas_list):
        ...
        ax.plot(mean_predicted_value, fraction_of_positives, 's-',
                label=clf_names[i], color=color, lw = 3)
    ...
    ax.grid(color = '#cccccc', axis = 'y', alpha = 0.6)
    ax.set_xticks(x, color = '#525252',fontfamily = 'serif')
    ax.set_yticks(x, color = '#525252',fontfamily = 'serif')
...
def plot_cumulative_gain(y_true, y_probas, title='Cumulative Gains Curve',
                         ax=None, figsize=(7,7), title_fontsize=18,
                         text_fontsize = 14, bbox_to_anchor = (1.3,.4), **kwargs):
    ...
    ax.plot(percentages, gains1, lw=3, label='Class {}'.format(classes[0]), color = 'red')
